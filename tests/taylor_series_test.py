...
class TaylorSeriesTestCase(unittest.TestCase):
    ...
    def test_accuracy_on_big_count(self):
        ...
        result = round(self.taylor_series.exponential_functionA(value, count), 3)
    ...
    def test_accuracy_on_small_count(self):
        ...
        expected_result = 148
        result = round(self.taylor_series.exponential_functionA(value, count), 0)
        self.assertEqual(result, expected_result, "Bad accuracy for small count")

    def test_wrong_count_exception(self):
        value = 5
        count = -1
        self.assertRaises(Exception, self.taylor_series.exponential_functionA(value, count))
